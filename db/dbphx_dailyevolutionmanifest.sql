...
--
-- Normalizes shardinstance rows into Phoenix steward-day bins.
----------------------------------------------------------------------
...
--
-- Aggregates lifeforcetraitshard per steward-day.
----------------------------------------------------------------------
...
--
-- Uses econetstaketerminalbatch2026q2phx.
----------------------------------------------------------------------
...
--
-- Computes daily EcoUnit share per steward vs region total.
----------------------------------------------------------------------
...
--
-- Primary composition layer. It joins:
...
--
-- Non-actuating filters:
--   - regioncode = 'Phoenix-AZ-US'
--   - roh_ok_day = 1 (RoH <= 0.30)
--   - no Lyapunov violations
--   - kerdeployable = 1 for contributing shards
----------------------------------------------------------------------

CREATE VIEW IF NOT EXISTS vphx_daily_evolution_manifest AS
WITH lyapunov_violations AS (
    SELECT DISTINCT
        si.stewarddid  AS stewarddid,
        si.regioncode  AS regioncode,
        date(si.tsendutc) AS day
...
WHERE k.regioncode = 'Phoenix-AZ-US'
  ...
  AND NOT EXISTS (
        SELECT 1
        FROM lyapunov_violations AS lo
