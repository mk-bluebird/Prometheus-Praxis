// filename: eco-net-bee/src/bee_corridor_provider.rs
// destination: mk-bluebird/Prometheus-Praxis (mono-repo), crate `eco-net-bee`
...
/// This is a readonly, non-actuating data structure that mirrors ecosafety
/// corridor semantics for the bee plane.
...
/// This is designed to be serializable as JSON and included in MCP / REST responses.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
///
/// Implementations:
/// - `BeeCorridorProviderMock` for CI / unit tests (pure in-memory).
/// - `BeeCorridorProviderOnChain` for live, on-chain backends.
...
impl BeeCorridorProvider for BeeCorridorProviderMock {
    ...
    fn validate_sunflower_placement(
        ...
        // A_min is the minimum uninterrupted habitat area required.
        let a_min = envelope.min_habitat_area_m2;
        // Proposed habitat loss Δ.
        let delta = request.delta_habitat_loss_m2;

        // Check basic invariants: Δ must not exceed remaining habitat.
        // Remaining habitat after placement: A_remaining = A_min - Δ.
        ...
        // Derived maximum density D_max = 1 / A_min.
        ...
        // Use the stricter of corridor-configured max density and formula.
        let d_max = envelope
            .max_sunflower_density
            .min(d_max_formula);

        // Compute proposed density contribution for this Sunflower.
    ...
    C: BeeCorridorBackendClient + Send + Sync + 'static,
{
    ...
    fn validate_sunflower_placement(
        ...
        // Delegate to the same deterministic logic as the mock, but feed in
        // live envelopes and ledger data from the backend.
        let envelope_opt = self.get_envelope(&request.cell_id);
        if envelope_opt.is_none() {
            return PlacementJustification {
                allowed: false,
        ...
        // Optionally, check existing ledger entries for this cell / asset.
