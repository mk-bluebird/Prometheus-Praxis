...
## III. Critical Gaps That Must Be Addressed

### Gap 1: Approved Cryptographic Registry ❌ MISSING

**The Problem:** The project establishes a blacklist but does NOT provide a complete, signed, versioned allow-list of approved cryptographic primitives.

**Required Artifact:**
```
crypto/approved-primitives.v1.cbor
```

**Required Fields Per Entry:**
- `primitive_id`, `primitive_family`, `purpose`
- `implementation_name`, `implementation_version`
- `parameter_profile`, `output_length`, `security_level`
- `test_vector_set_id`, `status`, `approved_at`
- `review_record`, `successor_primitive_id`, `minimum_verifier_version`

**Impact:** Without this, the system cannot proceed securely because:
- Merkle tree construction requires a specified hash
- Consent epoch tree requires a specified hash
- ZK proof system requires a specified hash
- Fuzzy extractor security proof requires a specified hash

### Gap 2: Test Vector Sets ❌ MISSING

Required for every approved primitive:
- Canonical serialization input bytes
- Domain tag bytes, digest output bytes
- Public key bytes, signature bytes
- Positive verification, wrong-domain rejection
- Wrong-algorithm rejection, modified-payload rejection
- Modified-witness rejection, old-version verifier compatibility

### Gap 3: Coefficient Registry ⚠️ SPECIFIED BUT NOT IMPLEMENTED

**What Exists:** Schema and graph structure.

**What's Missing:**
- Append-only Merkle-linked DAG of releases
- Drift detection infrastructure (`Dθ > κθ ∨ Dpred > κpred ∨ Dcoverage > κcoverage ⇒ Audit`)
- Causal DAG specification
- Prior-predictive checks, posterior-predictive checks
- Leave-one-study-out validation

### Gap 4: Neurobiometric Authorization Factor ⚠️ RESEARCH-ONLY

**What Exists:**
- Phase-locking value feature extraction formula
- Fuzzy extractor framework (Gen/Rep)
- Metric space definition, security notion

**What's Missing:**
- Pre-registered dataset
- Cross-session stability measurements
- False accept/reject rates
- Min-entropy estimation
- Helper data leakage analysis
- Replay resistance implementation

**Critical Warning:** From EEG authentication literature: "Same-user signals vary across time... task-imitation/impersonation concerns and unresolved questions about uniqueness at larger scale."

### Gap 5: Adjunction Proof Object ❌ MISSING

The claim: A suspected adjunction between `ObsTopo` and `ArtifactQuality` categories.

**Missing Proof Components:**
- Source category definition, target category definition
- Source object schema, target object schema
- Morphism definitions
- Functor L definition, Functor U definition
- Unit natural transformation, counit natural transformation
- Privacy noninterference proof, provenance preservation proof
- Consent scope monotonicity proof, no person ranking proof
- Triangle identities: `ε_{L(X)} ∘ L(η_X) = id_{L(X)}`, `U(ε_Y) ∘ η_{U(Y)} = id_{U(Y)}`

### Gap 6: Independent Review Protocol ⚠️ PARTIALLY SPECIFIED

**What Exists:**
- Reviewable predicate: `SchemaValid ∧ IntegrityValid ∧ ProtocolReconstructable ∧ ControlsDeclared ∧ UncertaintyDeclared ∧ RawHumanObservationsExcluded`

**What's Missing:**
- Formal reviewer credentialing policy
- Independent verification key governance
- Dispute resolution mechanism
- Appeal process specification
- Conflict-of-interest disclosure requirements

### Gap 7: Quantum-Resistant Migration Path ❌ NOT ADDRESSED

**Required:**
```
algorithm_profile_id, security_level, canonicalization_profile, domain_tag
implementation_version, test_vector_set, independent_review_reference
migration_successor, verification_support_window
```

### Gap 8: Measurement Lattice Implementation ⚠️ SPECIFIED BUT NOT IMPLEMENTED

```
L = ⨆_{k∈K} L_k where K = {glucose, protein, lipid, CRP, IL6, TNFα}
Compatible(a,b) ⟺ kind(a) = kind(b) ∧ dimension(a) = dimension(b)
```

**Critical Rule:**
```
Compatible(GlucoseMass, ProteinMass) = false
Compatible(IL6Concentration, TNFAlphaConcentration) = false
```
Even though both IL-6 and TNF-α may be reported in pg/mL—they are **NOT** interchangeable.
